...
from wrg_control.core import build_system_status


def test_import_cli_module() -> None:
    module = importlib.import_module("wrg_control.cli")
...
def temp_repo_root() -> Path:
    ...
    tmpdir = base_tmp / f"wrg_control_{uuid.uuid4().hex}"
...
def test_build_system_status_counts(temp_repo_root: Path) -> None:
    ...
    report = build_system_status(repo_root)
    statuses = {app["name"]: app["status"] for app in report["apps"]}

    assert report["totals"]["apps"] == 3
