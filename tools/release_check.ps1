...
function WRG-RunReleaseCheckForApp([string]$appName, [string]$appRoot) {
  ...
  try {
    ...
    # Smoke import only if package name seems to match app folder name
    $pkgDir = Join-Path $appRoot ("src\" + $appName)
    $pkgFile = Join-Path $appRoot ("src\" + $appName + ".py")

    if ((Test-Path -LiteralPath $pkgDir) -or (Test-Path -LiteralPath $pkgFile)) {
      $code = "import importlib; importlib.import_module('$appName'); print('IMPORT_OK')"
      WRG-RunDirect "smoke import" @($venvPy, "-c", $code) @(0)
    }
    else {
      WRG-Warn "Skipping smoke import; package import name may differ from app folder name."
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($testsDir) -and (Test-Path -LiteralPath $testsDir -PathType Container)) {
      try {
        & $venvPy -m pip install -q pytest | Out-Null
      }
      catch {
